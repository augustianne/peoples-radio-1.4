...
abstract class BaseTrack extends BaseObject  implements Persistent {
	...
	/**
	 * @var        TrackVote one-to-one related TrackVote object
	 */
	protected $singleTrackVote;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = TrackPeer::NUM_COLUMNS - TrackPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->singleTrackVote !== null) {
				if (!$this->singleTrackVote->isDeleted()) {
						$affectedRows += $this->singleTrackVote->save($con);
				}
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = TrackPeer::doValidate($this, $columns)) !== true) {
				...
				if ($this->singleTrackVote !== null) {
					if (!$this->singleTrackVote->validate($columns)) {
						$failureMap = array_merge($failureMap, $this->singleTrackVote->getValidationFailures());
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			$relObj = $this->getTrackVote();
			if ($relObj) {
				$copyObj->setTrackVote($relObj->copy($deepCopy));
			}
	...
	/**
	 * Gets a single TrackVote object, which is related to this object by a one-to-one relationship.
	 *
	 * @param      PropelPDO $con
	 * @return     TrackVote
	 * @throws     PropelException
	 */
	public function getTrackVote(PropelPDO $con = null)
	{

		if ($this->singleTrackVote === null && !$this->isNew()) {
			$this->singleTrackVote = TrackVotePeer::retrieveByPK($this->id, $con);
		}

		return $this->singleTrackVote;
	}

	/**
	 * Sets a single TrackVote object as related to this object by a one-to-one relationship.
	 *
	 * @param      TrackVote $l TrackVote
	 * @return     Track The current object (for fluent API support)
	 * @throws     PropelException
	 */
	public function setTrackVote(TrackVote $v)
	{
		$this->singleTrackVote = $v;

		// Make sure that that the passed-in TrackVote isn't already associated with this object
		if ($v->getTrack() === null) {
			$v->setTrack($this);
		}
	...
	{
		if ($deep) {
			if ($this->singleTrackVote) {
				$this->singleTrackVote->clearAllReferences($deep);
			}
		} // if ($deep)
